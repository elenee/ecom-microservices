...
export class OrderService {
    ...
    try {
      for (const item of cart.items) {
        await firstValueFrom(
          this.productClient.send('decrement_stock', { productId: item.productId, quantity: item.quantity })
        );
  ...
  async cancelOrder(userId, orderId) {
    ...
    for (const item of order.items) {
      await firstValueFrom(
        this.productClient.send('restock', { productId: item.productId, quantity: item.quantity })
      )
    }
  ...
  async handlePaymentFailure(orderId: string) {
    ...
    for (const item of order.items) {
      this.notificationClient.send('restock', { productId: item.productId, quantity: item.quantity })
    }
